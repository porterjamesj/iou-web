...
def flows2graph(flows):
    """Given a set of n flows for a connected graph, construct the simplest
    possible graph that satisfies their values. (this graph will have n-1 edges)
    """
...
def partition(flows,n=2):
    ...
    # if there are no zero-sum subsets, first check if we have found all subsets
    if n >= len(flows)//2:
